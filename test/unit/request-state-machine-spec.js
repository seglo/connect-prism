...
describe('mock request state machine', function() {
  ...
  it('should record filenames and append a sequential number to the name path of a file', function() {
    assert.equal(rsmachine.iteratePath(filename), 'myfile_0.json');
    assert.equal(rsmachine.iteratePath(filename), 'myfile_1.json');
    assert.equal(rsmachine.iteratePath(filename), 'myfile_2.json');
    assert.equal(rsmachine.iteratePath(filename), 'myfile_3.json');
    assert.equal(rsmachine.iteratePath(filename), 'myfile_4.json');
  });


  it('should provide a sequenzed filename for a request several times without increasing the sequence', function() {
    rsmachine.reset();
    assert.equal(rsmachine.iteratePath(filename), 'myfile_0.json');
  ...
  it('should be resetable for a single file', function() {
    rsmachine.iteratePath(filename2);
    rsmachine.reset(filename);
    // reset
    assert.equal(rsmachine.iteratePath(filename), 'myfile_0.json');    
    //not reset
    assert.equal(rsmachine.iteratePath(filename2), 'mysecondfile_1.json');    
  });
  ...
  it('should be resetable for every file', function() {
    rsmachine.reset();
    assert.equal(rsmachine.iteratePath(filename), 'myfile_0.json');
    assert.equal(rsmachine.iteratePath(filename2), 'mysecondfile_0.json');    
  });
