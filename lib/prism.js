...
function Prism(prismManager, logger, urlRewrite, httpEvents, proxy, mock, record, mockRecord, prismUtils) {
  var defaultConfig = {
    port: 80,
    https: false,
    mode: 'proxy',
  ...
  this.create = function(config) {
    ...
    if (validate(config)) {
      ...
      // handle errors from destination server
...
di.annotate(Prism, new di.Inject(PrismManager, Logger, UrlRewrite, HttpEvents, PrismProxy, Mock, Record, MockRecord, PrismUtils));
