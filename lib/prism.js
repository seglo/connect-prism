...
function Prism(prismManager, logger, urlRewrite, httpEvents, proxy, mock, record, mockRecord, prismUtils, clearMocks) {
  ...
  function validate(config) {
    ...
    if (mode !== 'proxy') {
      ...
      if (config.mocksPath.constructor === Array) {
	config.mocksPath.map(function(path){
	  if (_.isUndefined(path) || !grunt.file.isDir(path)) {
            grunt.file.mkdir(path);
            logger.warn('Mocks path did not exist \'' + path + '\'.  Created.');
	  }
	});
      }
