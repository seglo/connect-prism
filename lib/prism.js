...
prism.handleRequest = function(req, res, next) {
	...
	if (proxy) {
		...
		if (proxy.config.mode === 'read') {
			var pathToResponse = path.join(proxy.config.mocksPath, encodeURIComponent(req.url));
...
prism.serializeResponse = function(res, data, proxyOption) {
	var requestFilename = encodeURIComponent(res.req.path);
	var cachedResponse = {
	...
	var inProgressPath = finalPath + '.inProgress';

	// write file async to disk.  overwrite if it already exists.  prettyprint.
	fs.writeFile(inProgressPath, JSON.stringify(cachedResponse, true, 2), function(err) {
		if (err) {
			throw err;
		}
