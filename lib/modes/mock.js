...
function Mock(logger, prismUtils, mockFilenameGenerator, responseDelay) {
  ...
  function getMockPath(req, res, prism) {
    ...
    if (existingPaths.length > 0) {
      mockResponse(existingPaths[0], prism, req, res);
    } else {
      mockFilenameGenerator.createPathForFileIfNonExistent(paths[0]);
      write404(req, res, paths[0]);
      serializeEmptyMock(prism, req, paths[0]);
