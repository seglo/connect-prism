...
function MockFilenameGenerator() {
  /**
  ...
  this.getMockPath = function(prism, req) {
    var fn = prism.config.mockFilenameCallback || defaultMockFilename;
    ...
    return path.join(prism.config.mocksPath, prism.config.name, responseFilename + '.json');
  };
