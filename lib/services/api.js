...
function Api(prismManager, prism, prismUtils, mock, clearMocks) {
  ...
  this.handleRequest = function(req, res, next) {
    ...
    return router.dispatch(req, res/* *** Can't handle error and close response if this isn't a valid request, function(err) {
      if (err) {
...
di.annotate(Api, new di.Inject(PrismManager, Prism, PrismUtils, Mock, ClearMocks));
